...
WEBHOOK_URL=""
[ -n "$WEBHOOK_VAR" ] && WEBHOOK_URL="${!WEBHOOK_VAR:-}"
[ -n "$WEBHOOK_URL" ] || WEBHOOK_URL="${DEFAULT_WEBHOOK_URL:-}"

case "$FORMAT" in
    plain)     EXT="sql";  PGDUMP_FMT_FLAG="-Fp" ;;
    directory) EXT="dir";  PGDUMP_FMT_FLAG="-Fd" ;;
    *)         EXT="dump"; PGDUMP_FMT_FLAG="-Fc" ;;
esac
...
pg_dump -h "$PG_HOST" -p "$PG_PORT" -U "$PG_USER" -d "$PG_DATABASE" \
    ...
    || fail_db "$DB_ID" "pg_dump retornou erro" "$WEBHOOK_URL"

if [ "$DO_STRUCT_CHECK" = "true" ] && [ "$FORMAT" != "plain" ]; then
    pg_restore --list "$FILEPATH" >/dev/null 2>&1 \
        || fail_db "$DB_ID" "dump gerado é inválido (pg_restore --list falhou)" "$WEBHOOK_URL"
...
[ "$DEST_COUNT" -gt 0 ] || fail_db "$DB_ID" "nenhum destino configurado (destinations: vazio)" "$WEBHOOK_URL"
[ "$FAILED_DEST" = "0" ] || fail_db "$DB_ID" "um ou mais destinos falharam no upload (backup local preservado em ${FILEPATH})" "$WEBHOOK_URL"
...
# Retenção remota (apenas destinos rclone; OCI PAR usa Lifecycle Policy no bucket)
apply_remote_retention "$DB_ID"

if [ "$DO_TEST_RESTORE" = "true" ]; then
    run_test_restore "$DB_ID" "$FILEPATH" \
        || fail_db "$DB_ID" "verificação de restore de teste falhou" "$WEBHOOK_URL"
fi

log "$DB_ID" "backup finalizado com sucesso: ${FILENAME}"
send_webhook "$WEBHOOK_URL" "✅ [${DB_ID}] backup concluído: ${FILENAME} (${SIZE})"
